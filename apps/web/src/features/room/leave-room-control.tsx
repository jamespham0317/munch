 ...
 * irreversible, so a branded ConfirmModal gates the action before the mutation fires (replacing the
 * native confirm). The leave copy is context-aware: in the lobby a member can still rejoin (the
 * roster only freezes once a session starts — docs/04 §3.2), so the "can't rejoin" warning is shown
 * only on the session screen. The modal stays open with a spinner while the mutation runs and closes
 * on error (success routes home). Thin by design — all the server logic and routing live in
