...
/**
 * Keepalive + cosmetic presence for a room surface (lobby or session), Phase 4.7. Replaces the
 * old sticky `setPresence(true)` hack. Three concerns, all gated on a resolved `memberId`:
 *
 ...
 *      (focused from `visibilitychange`) and expose the synced `Map<memberId,{focused}>`. Purely
 *      presentational — ephemeral, zero DB writes, NEVER read by matchmaking (CLAUDE.md §2.3/§3).
...
export function useRoomPresence(
  ...
  const [presence, setPresenceMap] = useState<
    Map<string, { focused: boolean }>
  >(() => new Map());
  // Latest focus state, read by the subscribe + visibility handlers without re-subscribing.
