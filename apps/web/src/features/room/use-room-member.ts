...
/**
 * Resolve the caller's own ACTIVE member row for a room (CLAUDE.md §4 — no domain logic, just the
 * lookup every room surface shares). getRoomMembers returns only active members (left_at IS NULL,
 * Phase 4.7), so a member who left or was auto-removed drops out of `members` and `member` goes
 * null — that absence is how a surface detects its own removal (see useRemovedRedirect).
 *
