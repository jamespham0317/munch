 ...
 * Phase 4.7. Removal surfaces as the caller's own member row leaving the ACTIVE list
 * (getRoomMembers filters left_at IS NULL) — i.e. `memberId` going null after we'd seen it.
 *
...
export function useRemovedRedirect(opts: {
  ...
  useEffect(() => {
    ...
    router.replace("/?notice=disconnected");
