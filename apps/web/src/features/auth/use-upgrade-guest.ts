...
/**
 * Guest → account upgrade (docs/04 §2; the retention hook): step 1 links an email to the
 * current anonymous user and emails a 6-digit OTP; step 2 confirms it and writes the profile.
 * The user_id is unchanged, so the guest keeps their room membership (CLAUDE.md §3). Endpoint
 * shapes live in @munch/api-client (CLAUDE.md §4); the safe ApiError message is rethrown.
 */
