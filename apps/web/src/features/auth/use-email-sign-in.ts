...
/**
 * Fresh email-account sign-in (docs/04 §2): step 1 emails a 6-digit OTP, step 2 verifies it.
 * Two mutations so the panel can drive an email → code flow. All endpoint shapes live in
 * @munch/api-client (CLAUDE.md §4); the safe ApiError message is rethrown so TanStack Query
 * exposes it. This signs in as a NEW user — guests upgrading in a room use useUpgradeGuest.
 */
