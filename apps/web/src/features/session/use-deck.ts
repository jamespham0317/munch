...
/**
 * One-shot read of the session's cached deck. The deck is immutable for the session
 * (per-session caching, CLAUDE.md §2.1), so `staleTime: Infinity` keeps a single fetch
 * pinned for the lifetime of the screen. A swipe NEVER refetches this — that's the
