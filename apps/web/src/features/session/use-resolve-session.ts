...
/**
 * Host-only resolution flow (docs/04 §3.9): accept the top pick or widen the search. Both
 * actions go through the resolve-session Edge Function — widen needs the server-only provider
 * key, and accept_top rides along on the same endpoint (CLAUDE.md §2.1). The discriminated
 * request is passed straight through; the server validates and dispatches by `action`.
 *
 *   * accept_top → on success route to /result. The Edge Function also inserts the matches
 *     row, so co-members land on the same result screen via subscribeSession's match event;
 *     navigating here just makes it instant for the host.
