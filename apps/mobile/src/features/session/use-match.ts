...
/**
 * Result-screen read (RN parity with apps/web's useMatch): the session's match plus
 * the matched DeckRestaurant. Both swipe-screen entry paths (the swiper's submit_swipe
 * response and a co-member's realtime SessionMatchEvent) seed this cache key before
 * navigating, so a fresh fetch only runs on direct/refresh entry into the result route.
 */
