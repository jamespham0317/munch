...
/**
 * Read the room's latest non-terminal session under RLS (RN parity with apps/web's
 * useActiveSession). Used by the lobby to detect a host start and route everyone in,
 * and by the session screen to pull the session's snapshotted radius for the
 * local-only filter slider. A null result means no session is in flight.
 */
