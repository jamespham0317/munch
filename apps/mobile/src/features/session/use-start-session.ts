...
/**
 * Host-only start-session flow (RN parity with apps/web's useStartSession): invoke
 * the start-session Edge Function (the only path that touches the provider —
 * CLAUDE.md §2.1), then route the host to the swipe screen with the new session id.
 * Non-host members land on the same screen via the lobby's session-subscription
 * auto-route, not through this mutation.
 *
  ...
  return useMutation<StartSessionResponse, Error, StartSessionRequest>({
    ...
    onSuccess: (data) => {
      router.push({
        ...
        params: { roomId, sessionId: data.session.id },
      });
    },
  });
}
