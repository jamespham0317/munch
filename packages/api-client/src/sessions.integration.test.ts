...
import { startSession } from "./endpoints/sessions";
import { submitSwipe } from "./endpoints/swipes";
import type { ClientResult } from "./errors";

/**
 * Phase 2 integration tests for the core mechanic against a LOCAL Supabase, run through the
 * real api-client functions so the snake↔camel + error mapping are exercised end-to-end. The
 * highest-risk Phase-2 logic lives here: the server-authoritative unanimous match check
 * (incl. member-leaves-mid-session), submit_swipe idempotency, the host-leave session-cancel,
 * and the realtime match event (CLAUDE.md §7).
 *
 ...
 * (matching.test.ts, swipes.test.ts, errors.test.ts) always run. To run these locally:
 *
 ...
 *
 * The start_session block ALSO needs the Edge Function served with the FakeProvider (so tests
 * never hit the real provider — CLAUDE.md §7). Opt in with MUNCH_TEST_EDGE=1 and, in a second
 * terminal:
 *
 *   supabase functions serve start-session --env-file supabase/functions/.env.test
 *
