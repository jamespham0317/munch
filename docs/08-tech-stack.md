...
- **Maps/geo:** lightweight distance/geo math in `@munch/core` (incl. the radius→zoom fit and
  fixed-ring sizing helpers in `geo.ts`); provider supplies restaurant coordinates. The **anchor-selection
