...
**Project:** Munch
**Source:** `docs/11-ui-roadmap.md` §4 (Phase C) — expands the "UI polish" bullet of
...
Phase C puts the **"Munch Visual Language"** (`docs/09-design-system.md`) live in the browser and
...
component declares a match or calls the provider (`09-design-system.md` §9, CLAUDE.md §2/§4).

### Phase C maps to the roadmap bullets + exit criteria (`11-ui-roadmap.md` §4)
...
- Responsive nav: bottom **TabBar** at mobile widths → top/side nav at desktop (`10-pages.md` §2)
  + the Discover placeholder → Prompt 3
- Replace the remaining inline `style={}` usages; reskin every route in `10-pages.md`
...
- Web styling is **Tailwind v4**; its theme is **seeded from `@munch/ui` tokens** (CSS custom
  ...
  app (`09-design-system.md` §3) — so the `@theme`/CSS-var values must be **derived from
...
- **No `react-native-web`.** Component *implementations* are web-native (DOM); they mirror the
  Phase B mobile primitives' **API surface and look**, not their RN code (`09-design-system.md` §3).
...
- **Drop the swipe "save/super-like" (middle bookmark) button** — v1 is like/pass only; the
  mockup's middle button is intentionally not built (`09-design-system.md` §8, `11-ui-roadmap.md` §7).
...
- **Distance is shown in km** throughout (matching the Phase B mobile decision; the mockups are
  internally inconsistent on units — `11-ui-roadmap.md` §3, CLAUDE.md §9). Keep web and mobile
  consistent.
- **Auth is outside-a-room only and lives on the Profile destination**, not on the Welcome/Home
  screen (`10-pages.md` §2/§3.1/§3.2). The current web home renders `<AuthPanel>` inline; Phase C
...
bottom bar at mobile widths and a top/side nav at desktop; every route in `10-pages.md` §3 visually
...
remains for layout; and `09-design-system.md`/`10-pages.md`/`11-ui-roadmap.md` describe the web app as
shipped.
...
- Read CLAUDE.md (root) and, for this phase, docs/09-design-system.md (§3 platform mapping/packaging,
  §4 color, §5 typography, §6 spacing/radii/elevation, §7 primitives, §8 Munch-specific patterns,
  §9 invariants, §10 a11y), docs/10-pages.md (the screen inventory + per-page invariants, esp. §2
  navigation shell and §4 cross-cutting states), and docs/11-ui-roadmap.md (§4 Phase C, §7
  constraints).
- This is Phase C (Web reskin) of the UI work. The app is FUNCTIONALLY COMPLETE through Phase 4.5,
  ...
  stay in @munch/core (09-design-system.md §9, CLAUDE.md §2/§4).
- Consume tokens from @munch/ui. The Tailwind v4 theme is SEEDED FROM @munch/ui (CSS custom
  ...
  re-define the palette in the app (09-design-system.md §3). Use the SEMANTIC ROLES (brand, heat,
  surface, text, …), not raw hex. Where a literal value is unavoidable in CSS, it must be derived
  from / checked against @munch/ui.
- The mockups are the Stitch "Munch Visual Language" screens (titles match 10-pages.md §3). Where a
  mockup and the docs disagree, the docs win — specifically: build like/pass only and DROP the
  swipe middle bookmark button (09-design-system.md §8); use three destinations Discover·Match·Profile
  (10-pages.md §2) regardless of any single mockup's bar; show distance in km; auth lives on Profile,
  not the Welcome/Home screen (10-pages.md §2/§3.1). Match the Phase B mobile result where the mockup
  is silent.
...
serves the brand palette and font and the gates stay green. Reference: docs/09-design-system.md §3
(platform mapping: web seeds Tailwind @theme from @munch/ui), §4 color, §5 typography, §6
spacing/radii/elevation; 11-ui-roadmap.md §4.
...
- A theme that DERIVES from @munch/ui — do NOT hand-copy hex/sizes. Preferred: a tiny generation
  ...
  @munch/ui counterpart — single source of truth is non-negotiable, 09-design-system.md §3.)
...
- Establish the centered desktop container convention (max-width 1200px, 48px margins,
  09-design-system.md §6) as a reusable layout utility/class for later prompts to apply — do not yet
...
Reference: docs/09-design-system.md §7 (primitives), §6 (radii/elevation/press affordance), §10
(focus-visible, 44px targets, reduced-motion); 10-pages.md (where each is used); the Phase B mobile
...
(09-design-system.md §10). Export everything from a barrel (components/ui/index.ts). Add no new
state library and no react-native-web.

Done when: typecheck/lint/build pass; each primitive renders matching 09-design-system.md §7 (verify
...
```
Goal: introduce the three-destination navigation shell from 10-pages.md §2 using the TabBar
...
(including the deep link room/join/[code]) keep working. Reference: 10-pages.md §2 (navigation shell
+ tab→destination table) and §3.9 (Discover); 09-design-system.md §6 (responsive nav, 1200px
container) and §7 (TabBar). Depends on Prompts 1–2.

Destination → route (10-pages.md §2):
...
- Add a Next App Router ROUTE GROUP (e.g. app/(tabs)/) with a layout.tsx that renders the
  ...
  - Discover → a NEW placeholder page ("Under Construction", decorative amber/heat circles,
    "We're cooking up something special.") per 10-pages.md §3.9 — styled coming-soon state, no data.
...
loading/empty/error states with the primitives (10-pages.md §4). Reference: 10-pages.md §3.1 (Welcome),
§3.2 (Auth/Profile + History), §3.3 (Create Room), §3.4 (Join), §4 (cross-cutting states);
09-design-system.md §4–§8; the Stitch mockups "Welcome to Munch", "Join with Name and Code",
...
- Welcome (app/page.tsx): "Ready to eat?" header; a large amber "Create a Room" Card (with the +
  ...
  icon rows. MOVE <AuthPanel> off this page — auth now lives on Profile (10-pages.md §2/§3.1). Wiring:
...
- Auth / Profile (features/auth/auth-panel, the Profile destination, app/auth/callback +
  ...
  save" state, never an error (10-pages.md §3.2 invariant). Reskin the callback/reset views to match.
...
- Each route: loading = skeletons/spinners in card shapes (no layout shift); errors mapped from
  {error:{code,message}} (10-pages.md §4); centered 1200px container at desktop, full-width at mobile.
...
reskin — treat the invariants as load-bearing. Reference: 10-pages.md §3.5 (Lobby), §3.6 (Session),
§3.7 (Match), §3.8 (Host Resolution), §4; 09-design-system.md §8 (Decision Card, swipe action row,
...
- Lobby (room/[roomId]/lobby/page.tsx, features/room/lobby-view, member-list, invite-panel):
  ...
  presence only (no per-member status text — v1, 09-design-system.md §8).
...
the phase closes green and the docs match reality (CLAUDE.md §1). Reference: 11-ui-roadmap.md §4
(Phase C exit) + §5 (Phase D verify checklist as the standard); 09-design-system.md §9 (invariants);
10-pages.md §2–§4 (nav shell, per-screen expectations, cross-cutting states). Depends on Prompts 1–5.
...
- Invariant check (presentation-only, 09-design-system.md §9): confirm NO provider call on swipe and
...
- Doc reconciliation: update 11-ui-roadmap.md so Phase C reads as delivered (its exit criteria met);
  reconcile 10-pages.md / 09-design-system.md with anything that shipped differently (e.g. a primitive
  name, the responsive nav structure, a state). Keep edits minimal and factual.

Done when: all four gate commands are green tree-wide; every route in 10-pages.md §3 visually matches
